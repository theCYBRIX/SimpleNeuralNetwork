...
public class Species<E extends RankedNeuralNetwork> implements Population<E>{
    ...
    private List<E> previousLeaderBoard = null; 
    private LinkedList<E> manualAdditions = new LinkedList<>(); 

    private Optional<Double> bestScore = Optional.empty();

    public Species(int numNetworks, Fraction parentFraction, Supplier<E> networkProvider, Collection<OffspringProvider<E>> offspringProviders, CompoundRatio distribution, Comparator<E> comparator) throws DimensionsMismatchException, IllegalArgumentException, NullPointerException{
    ...
    public Species(Fraction parentFraction, Supplier<E> networkProvider, Collection<OffspringProvider<E>> offspringProviders, CompoundRatio distribution, Comparator<E> comparator) throws DimensionsMismatchException, IllegalArgumentException, ArithmeticException, NullPointerException{
        this(1, parentFraction, networkProvider, offspringProviders, distribution, comparator);
    }

    public Species(Fraction parentFraction, Supplier<E> networkProvider, Collection<OffspringProvider<E>> offspringProviders, Comparator<E> comparator) throws DimensionsMismatchException, IllegalArgumentException, ArithmeticException, NullPointerException{
        this(1, parentFraction, networkProvider, offspringProviders, CompoundRatio.uniform(offspringProviders.size()), comparator);
    }

    public Species(int numNetworks, int numParentNetworks, Supplier<E> networkProvider, Collection<OffspringProvider<E>> offspringProviders, CompoundRatio distribution, Comparator<E> comparator) throws DimensionsMismatchException, IllegalArgumentException, ArithmeticException, NullPointerException{
        this(numNetworks, Fraction.of(numNetworks, numParentNetworks), networkProvider, offspringProviders, distribution, comparator);
    }

    public Species(int numNetworks, int numParentNetworks, Supplier<E> networkProvider, Collection<OffspringProvider<E>> offspringProviders, Comparator<E> comparator) throws IllegalArgumentException, NullPointerException{
    ...
    public void setSize(int numNetworks) {
    ...
    public void setSize(int numNetworks, int numParentNetworks) {
        this.totalNumNetworks = numNetworks;
    ...
    public void populateNewGeneration(){
        synchronized(newGenerationAlgorithm){
            ...
            previousLeaderBoard = Collections.unmodifiableList(new ArrayList<E>(leaderBoard));
    ...
    public ArrayList<E> getLeaderBoard(Comparator<E> comparator) {
        return getLeaderBoard(new ArrayList<>(previousLeaderBoard.size()), comparator);
    }

    public <T extends List<E>> T getLeaderBoard(T destination, Comparator<E> comparator) {
        destination.addAll(previousLeaderBoard);
    ...
    private class ParallelNewGeneration implements Runnable{
        ...
        public void run() {
            ...
            for(int i = 0; i < offspringProviders.size(); i++){
                final OffspringProvider<E> PROVIDER = offspringProviders.get(i);
                final int NUM_OFFSPRING = NETWORKS_PER_PROVIDER[i];
                Callable<Collection<E>> task = () -> PROVIDER.createOffspring(parentNetworks, NUM_OFFSPRING);
